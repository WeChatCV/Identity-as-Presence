...
class ConversionConfig:
    ...
    num_merge_workers: int = 2
    keep_temp_folder: bool = False
    strict: bool = True
    inject_missing_state: bool = False
    input_folder: str = ""
    output_folder: str = ""
...
def _handle_existing_conversion(checkpoint_dir):
    """Handle cases where latest_universal flag already exists"""
    print(f">> Handling existing conversion in {os.path.basename(checkpoint_dir)}")
    
    origin_exists = os.path.exists(os.path.join(checkpoint_dir, "pytorch_model_origin"))
    model_univ = os.path.join(checkpoint_dir, "pytorch_model_universal")
    model_normal = os.path.join(checkpoint_dir, "pytorch_model")
    
    # Case 1.1: Origin directory exists
    if origin_exists:
        if os.path.exists(model_normal):  # Case 1.1a
            print(">> Already has valid structure, do nothing")
        elif os.path.exists(model_univ):  # Case 1.1b
            print(">> Renaming *_universal to standard name")
            shutil.move(model_univ, model_normal)
    # Case 1.2: No origin directory exists
    else:
        if os.path.exists(model_normal):
            print(">> Initial conversion cleanup")
            # Rename original directory
            shutil.move(model_normal, os.path.join(checkpoint_dir, "pytorch_model_origin"))
            # Rename universal directory
            if os.path.exists(model_univ):
                shutil.move(model_univ, model_normal)

def _reorganize_dir(origin_folder, universal_folder):
    print("\n>> Reorganizing directories:")
    # Backup original directory
    shutil.move(origin_folder, origin_folder + "_origin")
    # Create standard directory name
    shutil.move(universal_folder, origin_folder)
    print(">> Conversion completed successfully")

def _move_random_states_files(src_dir: str):
    """
    Move all random_states_*.pkl files to a new directory
    
    :param src_dir: Source directory path
    :param dest_dir: Destination directory path
    """
    # Create destination directory if not exists
    dest_dir = os.path.join(src_dir, "random_states")
    Path(dest_dir).mkdir(parents=True, exist_ok=True)
    
    for filename in os.listdir(src_dir):
        # Match file pattern using basic string operations
        if filename.startswith("random_states_") and filename.endswith(".pkl"):
    ...
    # Summary output
    print("move_random_states_files done.\n")

def convert_universal_checkpoint(args: ConversionConfig, main_process: bool):
    done_file = os.path.join(args.checkpoint_folder, 'universal_ckpt.done')
    if not main_process:
        check_interval = 1  # seconds
        while not os.path.exists(done_file):
            time.sleep(check_interval)
        return
    # handle by main_prcess, other process hold on
    if os.path.exists(done_file):
        return
    _move_random_states_files(args.checkpoint_folder)
    universal_flag = os.path.join(args.checkpoint_folder, "latest_universal")
    if os.path.exists(universal_flag):
        _handle_existing_conversion(args.checkpoint_folder)
        return
    
    model_univ = os.path.join(args.checkpoint_folder, "pytorch_model_universal")
    model_orig = os.path.join(args.checkpoint_folder, "pytorch_model")
    args.input_folder = model_orig
    args.output_folder = model_univ
    print(f'args = {args}')
    _convert(args)
    if os.path.exists(universal_flag):
        _reorganize_dir(model_orig, model_univ)
